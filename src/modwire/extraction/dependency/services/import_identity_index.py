...
class ImportIdentityIndex:
    def __init__(self) -> None:
        self._exact: dict[tuple[str, str], set[FileId]] = {}
    ...
    def add(self, group: str, identity: str, file_id: FileId) -> None:
        self._exact.setdefault((group, identity), set()).add(file_id)
        if not identity:
    ...
    def exact(self, group: str, identity: str) -> set[FileId]:
        return set(self._exact.get((group, identity), ()))

    def suffix(self, group: str, identity: str) -> set[FileId]:
        if not identity:
