...
from ..map.application import MapApplication
...
class ReportApplication:
    ...
    def report(self, code_map: QueryableCodeMap, config: ArchitectureConfig) -> tuple[ReportNode, ...]:
        architecture_map = self.maps.load(code_map, config.boundaries)
        reports = (collector.collect(architecture_map, config) for collector in self.collectors)
