...
class ScanBenchmarkFixture:
    def __init__(self, root: Path, included_files: int, excluded_directories: int, excluded_files: int) -> None:
        self.root = root
    ...
    def create(self) -> None:
        ...
        for index in range(self.included_files):
            (included_root / f"example_{index:05d}.py").write_text(
                f"class Example{index:05d}:\n    pass\n",
                encoding="utf-8",
            )
        ...
        for directory_index in range(self.excluded_directories):
            ...
            for file_index in range(self.excluded_files):
                (directory / f"example_{file_index:05d}.py").write_text("example_value = 1\n", encoding="utf-8")
        manifest = {
